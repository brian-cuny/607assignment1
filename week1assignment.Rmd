...
<div class='well'>
  I began by reading 2 csv files. The first contains the unaltered raw mushroom data while the second contains a lightly edited version of the list of names and abbreviations to be used with the list.
</div>

```{r}
names <- t(names)
names(mushrooms_raw) <- colnames(names)
head(mushrooms_raw[,1:5])
head(names[,1:5])
```

<div class='well'>
  I transformed names for easier processing and then used the column names to assign the columns of mushrooms_raw.
</div>
...
<div class='well'>
  I subset the data frame by selected classes and 3 other columns (odor, population and habitat). I stored this subset in mushrooms_reduced.
</div>
...
head(mushrooms_reduced)
```

<div class='well'>
  I loop over all the appropriate columns calling the method and this will substitute the abbrevation for the full name in every entry in the data frame.
</div>

<div class='alert alert-info'>
  In conclusion, the original mushrooms data has been read in, subset and updated with more readable information and is ready for further processing.
</div>
